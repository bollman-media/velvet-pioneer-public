// Loading State — Time Machine Immersive Gallery
...
(function () {
    ...
    // ── DOM ───────────────────────────────────────────────────────────────────
    const shell2       = document.getElementById('phone-shell-loading');
    if (!shell2) return; // guard: loading shell not in DOM
    ...
    const chatOverlay  = document.getElementById('lum-loading-chat-overlay');
    const chatHistory  = document.getElementById('lum-loading-chat-history');
    const immersive    = document.getElementById('lum-loading-immersive');
    const immClose     = document.getElementById('lum-loading-immersive-close');
    const theater      = document.getElementById('lum-loading-card-theater');
    const bgA          = document.getElementById('lum-loading-bg-a');
    const bgB          = document.getElementById('lum-loading-bg-b');
    const actionPill   = document.getElementById('lum-loading-action-pill');
    const posIndicator = document.getElementById('lum-loading-position-indicator');
    const promptLabel  = document.getElementById('lum-loading-prompt-label');
    const promptText   = document.getElementById('lum-loading-prompt-text');
    ...
    const PROMPT_TRAVEL_Y_PX = 16;   // Loading State swipe is depth-based, use subtle vertical offset
    ...
    const opt2Container = shell2.querySelector('#lum-loading-cards-container');
    ...
    const opt2Scroll = document.getElementById('lum-loading-cards-container');
    if (opt2Scroll) {
    ...
    function appendChatLoading(prompt) {
        ...
        t.innerHTML = `
            ...
            <div class="lum-chat-shimmer-box" style="background: rgba(255,255,255,0.05);"></div>`;
    ...
    async function loadImageWithProgress(imgUrl, imgElement) {
        // Hide image to start while it dictates DOM dimensions
        imgElement.style.opacity = '0';
        imgElement.src = imgUrl;

        // Wait a frame so the caller has time to append this img to the parent DOM
        ...
        // Phase 1: Blurry movement overlay
        const movementDiv = document.createElement('div');
        movementDiv.className = 'lum-loading-movement';
        if (parent) {
            parent.insertBefore(movementDiv, imgElement);
        }

        // --- Dynamic Color Extraction ---
        // Sample the image natively to color-match the loading movement aura
        const colorImg = new Image();
        colorImg.crossOrigin = "Anonymous";
        colorImg.onload = () => {
            const cvs = document.createElement('canvas');
            cvs.width = 3; cvs.height = 3; // 3x3 downsample
            const ctx = cvs.getContext('2d', { willReadFrequently: true });
            try {
                ctx.drawImage(colorImg, 0, 0, 3, 3);
                const d = ctx.getImageData(0, 0, 3, 3).data;
                // Sample Top-Left (index 0), Center (index 4), Bottom-Right (index 8)
                const c1 = `rgba(${d[0]}, ${d[1]}, ${d[2]}, 0.6)`;
                const c2 = `rgba(${d[16]}, ${d[17]}, ${d[18]}, 0.4)`;
                const c3 = `rgba(${d[32]}, ${d[33]}, ${d[34]}, 0.5)`;
                
                // Override the CSS gradient with the AI generated color palette
                movementDiv.style.background = `linear-gradient(135deg, ${c1}, ${c2}, ${c3})`;
            } catch(e) {
        ...
        try {
            // Measure network fetch to control duration of Phase 1
            const response = await fetch(imgUrl);
            const total = parseInt(response.headers.get('content-length'), 10) || 0;
            const reader = response.body ? response.body.getReader() : null;

            if (!reader || !total || total < 1000 || imgUrl.startsWith('data:')) {
                // Simulate Phase 1 artificial generation time
                await new Promise(r => setTimeout(r, 1800));
                if (reader) while (!(await reader.read()).done) {}
            } else {
                while (!(await reader.read()).done) {}
                await new Promise(r => setTimeout(r, 500)); // Base processing padding
            }
        } catch(e) {
            await new Promise(r => setTimeout(r, 1000));
        }

        // Phase 2: Crossfade the heavily blurred loaded layout
        const maxBlur = 30;
        imgElement.style.transition = 'none';
        imgElement.style.filter = `blur(${maxBlur}px) brightness(1.3)`;
        ...
        movementDiv.style.transition = 'opacity 0.8s ease-in-out';

        imgElement.style.opacity = '1';
        movementDiv.style.opacity = '0';

        await new Promise(r => setTimeout(r, 800));
        movementDiv.remove();

        // Phase 3: Blurry to Sharp
        imgElement.style.transition = 'filter 2s cubic-bezier(0.2, 0.8, 0.2, 1), transform 2s cubic-bezier(0.2, 0.8, 0.2, 1)';
        
        void imgElement.offsetWidth; // Trigger new transitions

        imgElement.style.filter = 'blur(0px) brightness(1)';
    ...
    function appendChatImage(dataUrl, prompt, index) {
        ...
        bubble.className = "lum-chat-bubble";
        bubble.textContent = prompt;
        t.appendChild(bubble);

        const wrap = document.createElement('div');
        wrap.className = "lum-chat-image-wrap";
        wrap.style.position = "relative";
        wrap.style.maxWidth = "370px";

        const img = document.createElement('img');
        img.loading = "lazy";
        img.alt = prompt;
        
        // Apply aspect ratio from prompt immediately, refine on load
        if (window.applyImageAspectRatio) {
            window.applyImageAspectRatio(wrap, img, prompt);
        }

        loadImageWithProgress(dataUrl, img);

        const btn = document.createElement('button');
        btn.className = "lum-loading-expand-btn";
        ...
        wrap.addEventListener('click', e => { if (!btn.contains(e.target)) open(); });
    }

    function scrollBottom() {
        const el = document.getElementById('lum-loading-chat-scroll');
    ...
    function buildCard(dataUrl, prompt) {
        ...
        card.className = 'lum-loading-card';
        const vp = document.createElement('div');
        vp.className = 'lum-loading-card__viewport';
        const img = document.createElement('img');
        img.className = 'lum-loading-card__img';
        img.draggable = false;
        img.alt = prompt;
        ...
        if (prompt && prompt.trim() !== '') {
            ...
            lbl.className = 'lum-loading-prompt-label';
            lbl.style.position = 'absolute';
            lbl.style.top = '24px'; /* Sit naturally at the top of the image frame */
            lbl.style.right = '24px';
            lbl.style.left = '24px';
            lbl.style.zIndex = '5';
            lbl.innerHTML = `<span class="lum-loading-prompt-label__text">${prompt}</span>`;
            vp.appendChild(lbl);
        }
    ...
    function buildLoadingCard() {
        const card = document.createElement('div');
        card.className = 'lum-loading-card is-loading-card';
        // Create viewport wrapper for loading content (consistent with regular cards)
        const vp = document.createElement('div');
        vp.className = 'lum-loading-card__viewport';
        vp.innerHTML = `
            <div class="lum-loading-loading-content" style="background: rgba(255,255,255,0.05);">
                <div class="lum-loading-loading-label-wrap">
                    <span class="lum-loading-loading-label">Generating image...</span>
                </div>
            </div>`;
        card.appendChild(vp);
        theater.appendChild(card);
    ...
    function layoutCards(targetIdx, animate) {
        ...
        cards.forEach((entry, i) => {
            ...
            el.style.opacity   = '';
            el.style.filter    = '';
            const lbl = el.querySelector('.lum-loading-prompt-label');
    ...
    function handleDragEnd() {
        ...
        if (isTap && cards.length > 1) {
            ...
            cards.forEach(entry => {
                ...
                const lbl = entry.el.querySelector('.lum-loading-prompt-label');
                if (lbl) lbl.style.filter = '';
            });
    ...
    const floatingInput2    = shell2.querySelector('#lum-loading-floating-input');
            ...
            generateFashionImage(prompt, style, new AbortController().signal)
                .then(dataUrl => {
                    ...
                    if (prompt && prompt.trim() !== '') {
                        ...
                        lbl.style.right = '24px';
                        lbl.style.left = '24px';
                        lbl.innerHTML = `<div class="lum-loading-prompt-label__text">${prompt}</div>`;
...
// ─────────────────────────────────────────────────────────────────────────────
// Intercepts two-finger pinch gestures on the lum-loading-immersive overlay.
...
(function initPinchZoom2() {
    ...
    function getCurrentCard() {
        return theater.querySelector('.lum-loading-card.is-current');
    }

    function getCurrentImg() {
        const card = getCurrentCard();
        return card ? card.querySelector('.lum-loading-card__img') : null;
    }
    ...
    //  patch the loading handleDragStart to guard with this flag.)
